# Adapt from https://github.com/deepseek-ai/FlashMLA/blob/main/tests/test_flash_mla_prefill.py
import dataclasses
import math
import random
import time
        ...
        results.append(
            {
                ...
                "s_q": p.s_q,
                "s_kv": p.s_kv,
                "latency_us": round(prefill_ans_time * 1e6, 3),
                "mfu": round(prefill_flops / gpu_tflops, 3),
            }
        )
...
if __name__ == "__main__":
    ...
    performance_cases = [
        ...
        for s_q in [4096]
        for s_kv in [4096, 8192, 16384, 32768, 65536, 131072]
        for topk in [2048]
    ]
    ...
    df.to_csv("dsa_prefill.csv", index=False)
